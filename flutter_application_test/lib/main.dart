  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        // 然后触发"热重载"（保存你的更改或在Flutter支持的IDE中按"热重载"按钮，或如果你使用命令行启动应用程序，则按"r"）。
        //
...
// 复位计数器
  ...
  Widget build(BuildContext context) {
    ...
    //
    // Flutter框架已经进行了优化，使得重新运行build方法变得快速，
    // 这样你就可以只重建需要更新的东西，而不是单独更改widget实例。
    return Scaffold(
      appBar: AppBar(
        // 尝试这个：尝试将这里的颜色更改为特定颜色（例如
        // Colors.amber），然后触发热重载以查看AppBar
        // 更改颜色，而其他颜色保持不变。
      ...
      body: Center(
        ...
        // 放在父级的中间。
        child: Column(
          // Column也是一个布局widget。它接受一个子项列表并将其
          // 垂直排列。默认情况下，它会根据其子项水平调整大小，
          // 并尝试与其父级一样高。
          //
          // Column具有各种属性来控制其如何调整大小以及
          // 如何定位其子项。这里我们使用mainAxisAlignment将子项垂直居中；
          // 这里的主轴是垂直轴，因为Columns是垂直的（交叉轴将是水平的）。
          //
          // 尝试这个：调用"调试绘制"（在IDE中选择"Toggle Debug Paint"
          // 操作，或在控制台中按"p"），以查看每个widget的线框。
